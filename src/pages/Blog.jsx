...
const Blog = () => {
  ...
  // Locales.js dosyasından çevirileri çek
  const t = siteContent[lang]?.pages?.blog || {};

  const [contentList, setContentList] = useState([]); 
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);

  // Veritabanından veri çekme (Aynı kalır)
  useEffect(() => {
  ...
  const getIcon = (type) => {
    switch (type) {
      case 'book': return BookOpen; case 'video': return Film; case 'music': return MusicIcon; default: return MessageSquare;
    }
  };

  const getColor = (type) => {
    switch (type) {
      case 'book': return 'bg-yellow-500'; case 'video': return 'bg-red-600'; case 'music': return 'bg-indigo-600'; default: return 'bg-blue-600';
    }
  };
