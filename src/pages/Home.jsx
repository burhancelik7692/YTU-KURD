...
const Home = () => {
  ...
  // İstatistikler (Statik ama etkileyici)
  const stats = [
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-gray-50">
        ...
        <section className="relative min-h-[85vh] flex items-center justify-center overflow-hidden pb-20 pt-20">
          <div className="absolute inset-0 z-0">
            ...
            {/* Daha koyu ve modern gradient */}
            <div className="absolute w-full h-full bg-gradient-to-b from-slate-900/90 via-slate-900/80 to-slate-900/95"></div>
          </div>

          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="max-w-5xl mx-auto text-center relative z-10 px-4">
            
            {/* Logo */}
            <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: "spring", stiffness: 260, damping: 20 }}>
              <img src="/logo.png" alt="YTU Kurdî" className="w-28 h-28 md:w-36 md:h-36 mx-auto mb-8 rounded-full shadow-2xl ring-4 ring-yellow-500/30" />
            </motion.div>
            ...
            <p className="text-lg md:text-xl text-slate-300 mb-10 max-w-2xl mx-auto font-light px-2">
              {t.heroSubtitle}
            </p>

            {/* Hızlı Erişim Butonları (YENİ) */}
            <div className="flex flex-wrap justify-center gap-4">
               <Link to="/ferheng" className="flex items-center gap-2 bg-white text-blue-900 px-8 py-4 rounded-full font-bold text-lg hover:bg-blue-50 transition shadow-lg shadow-white/10 transform hover:-translate-y-1">
                 <Search size={20} /> {lang === 'KU' ? 'Ferheng' : 'Sözlük'}
               </Link>
               <Link to="/listik" className="flex items-center gap-2 bg-yellow-500 text-blue-900 px-8 py-4 rounded-full font-bold text-lg hover:bg-yellow-400 transition shadow-lg shadow-yellow-500/20 transform hover:-translate-y-1">
                 <Gamepad2 size={20} /> {lang === 'KU' ? 'Bilîze' : 'Oyna'}
               </Link>
            </div>

            {/* İstatistik Barı (YENİ) */}
        ...
        {/* --- 2. GÜNCEL KARTLAR (ETKİNLİK & SÖZ) --- */}
        <section className="relative z-20 px-4 -mt-16 md:-mt-24 pb-16">
          <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-6">
            
            {/* Etkinlik Kartı */}
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="bg-blue-900 text-white rounded-3xl p-8 shadow-2xl border border-blue-800 flex flex-col justify-center relative overflow-hidden min-h-[280px]">
              ...
              <div className="flex justify-between items-start mb-6">
                <div className="flex items-center gap-3">
                  <div className="bg-yellow-500 p-2 rounded-xl text-blue-900 shadow-lg shadow-yellow-500/20"><Bell size={24} /></div>
                  <h3 className="text-xs font-bold text-blue-200 uppercase tracking-widest">{t.eventTitle}</h3>
                </div>
                {/* Durum Rozeti */}
                <span className="bg-blue-800 text-blue-200 text-xs px-3 py-1 rounded-full border border-blue-700">
            ...
            {/* Günün Sözü Kartı */}
            {dailyItem && (
              <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.2 }} className="bg-white rounded-3xl p-8 shadow-2xl border border-slate-100 flex flex-col justify-center relative min-h-[280px]">
                ...
                <div className="flex-1 flex flex-col justify-center relative z-10 pr-8">
                  <p className="text-2xl md:text-3xl font-black text-slate-800 mb-3 leading-snug">"{dailyItem.text}"</p>
                  <p className="text-lg text-slate-500 italic border-l-4 border-yellow-400 pl-4">{dailyItem.meaning}</p>
                </div>

                <Quote className="absolute bottom-6 right-6 text-slate-100 rotate-180" size={100} />
              </motion.div>
            )}
        ...
        <section className="py-16 md:py-24 px-4 bg-white relative z-10">
          <div className="max-w-7xl mx-auto">
            <div className="text-center mb-16">
              <h2 className="text-4xl font-black text-slate-900 mb-4">{t.featuresTitle}</h2>
              ...
              {features.map((feature, index) => (
                <motion.div key={feature.path} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: index * 0.1 }}>
                  <Link to={feature.path} className="block h-full">
                    <div className="bg-slate-50 rounded-3xl p-8 hover:shadow-2xl transition-all duration-300 hover:-translate-y-2 group border border-slate-100 h-full flex flex-col relative overflow-hidden">
                      ...
                      <h3 className="text-2xl font-bold text-slate-900 mb-3 group-hover:text-blue-700 transition-colors">{feature.title}</h3>
