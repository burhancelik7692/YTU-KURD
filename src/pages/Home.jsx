  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-gray-50">
        ...
        {/* --- HERO SECTION (Giriş) --- */}
        {/* DÜZELTME: min-h-screen ve pb-32 ekleyerek içeriğin kartların altında kalmasını engelledik */}
        <section className="relative min-h-[90vh] lg:min-h-screen flex items-center justify-center overflow-hidden pb-20 lg:pb-32">
          ...
          <motion.div 
            ...
            className="max-w-6xl mx-auto text-center relative z-10 px-4 pt-24"
          >
            <motion.img 
              ...
              src="/logo.png" 
              alt="YTU Kurdî" 
              className="w-28 h-28 md:w-40 md:h-40 mx-auto mb-6 rounded-full shadow-2xl ring-4 ring-white/20" 
            />
            ...
            <p className="text-lg md:text-xl lg:text-2xl text-slate-300 mb-8 max-w-2xl mx-auto font-light px-4">
              {t.heroSubtitle}
            </p>
            ...
            <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
               <a 
                 ...
                 className="inline-block bg-white text-blue-900 px-8 py-3 md:px-10 md:py-4 rounded-full font-bold text-base md:text-lg hover:bg-blue-50 transition shadow-lg shadow-white/10"
               >
                 {t.heroButton}
               </a>
            </motion.div>
        ...
        {/* --- BİLGİ KARTLARI (Geri Sayım & Günün Sözü) --- */}
        {/* DÜZELTME: Mobilde normal akış (mt-0), PC'de yukarı taşma (-mt-32) */}
        <section className="relative z-20 px-4 pb-12 -mt-12 lg:-mt-32">
          <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-6">
            
            {/* 1. KART: ETKİNLİK (BOŞ/PLACEHOLDER) */}
            <motion.div 
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              className="bg-blue-900 text-white rounded-3xl p-8 shadow-2xl border border-blue-800 flex flex-col justify-center relative overflow-hidden min-h-[260px]"
            >
              {/* Arka Plan Efekti */}
              ...
              <div className="flex items-center gap-3 mb-6">
                ...
                <h3 className="text-xs font-bold text-blue-200 uppercase tracking-widest">
                  {t.eventTitle || (lang === 'KU' ? 'Çalakiya Pêşerojê' : 'Sıradaki Etkinlik')}
                </h3>
              </div>

              <div className="flex-1 flex flex-col justify-center items-center text-center">
                <h2 className="text-3xl font-bold text-white mb-2">
                  {t.eventEmptyTitle || (lang === 'KU' ? 'Tê Ragihandin' : 'Duyurulacak')}
                </h2>
                <p className="text-blue-200 text-sm mb-6">
                  {t.eventEmptyDesc || (lang === 'KU' ? 'Ji bo çalakiyên nû li bendê bin.' : 'Yeni etkinlikler için beklemede kalın.')}
                </p>
                
                {/* BOŞ SAYAÇ (Görsel Placeholder) */}
            ...
            {dailyItem && (
              <motion.div 
                ...
                className="bg-white rounded-3xl p-8 shadow-2xl border border-slate-100 flex flex-col justify-center relative min-h-[260px]"
              >
                <div className="flex items-center gap-3 mb-6">
        ...
        <section className="py-16 md:py-24 px-4 bg-slate-50 relative z-10">
          <div className="max-w-7xl mx-auto">
            <motion.h2 initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="text-3xl md:text-4xl font-bold text-center text-blue-900 mb-16">
              {t.featuresTitle}
            </motion.h2>
        ...
        {/* --- CTA SECTION --- */}
        <section className="py-20 px-4 bg-blue-900 relative z-10">
