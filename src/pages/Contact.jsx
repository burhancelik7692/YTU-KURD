    ...
    <>
      ...
      <div className="min-h-screen bg-slate-50 dark:bg-slate-900 pt-24 pb-12 px-4 transition-colors duration-300">
        <div className="max-w-6xl mx-auto">
          ...
          {/* GERİ DÖN TUŞU BURADAN KALDIRILDI */}
              ...
              <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-xl p-8 border border-slate-100 dark:border-slate-700">
                ...
                <div>
                  ...
                  <div className="flex gap-4">
                    <a href="https://instagram.com/ytukurdi" target="_blank" rel="noreferrer" className="bg-slate-100 dark:bg-slate-700 p-3 rounded-xl text-slate-600 dark:text-slate-300 hover:bg-pink-100 hover:text-pink-600 transition">
                      <Instagram size={24} />
                    </a>
                    <a href="https://www.youtube.com/@ytukurdi" target="_blank" rel="noreferrer" className="bg-slate-100 dark:bg-slate-700 p-3 rounded-xl text-slate-600 dark:text-slate-300 hover:bg-red-100 hover:text-red-600 transition">
                      <Youtube size={24} />
                    </a>
                  </div>
                </div>
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.4 }} className="bg-white dark:bg-slate-800 rounded-3xl shadow-xl p-8 border border-slate-100 dark:border-slate-700 flex flex-col">
              ...
              {status === 'success' ? (
                ...
                <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="flex-1 flex flex-col items-center justify-center text-center p-8 bg-red-50 dark:bg-red-900/20 rounded-2xl border border-red-100 dark:border-red-800">
                  ...
                  <p className="text-red-700 dark:text-red-400">Mesaj gönderilemedi. Lütfen daha sonra tekrar deneyin.</p>
                ...
                <form ref={formRef} onSubmit={handleSubmit} className="space-y-4 flex-1">
                  <div>
                    ...
                    <input type="text" name="name" required className="w-full px-4 py-3 rounded-xl bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-600 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 dark:text-white outline-none transition" placeholder="..." />
                  </div>
                  <div>
                    <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">{content.form.email}</label>
                    <input type="email" name="email" required className="w-full px-4 py-3 rounded-xl bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-600 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 dark:text-white outline-none transition" placeholder="nav@mînak.com" />
                  </div>
                  <div>
                    <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">{content.form.message}</label>
                    <textarea name="message" required rows="5" className="w-full px-4 py-3 rounded-xl bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-600 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 dark:text-white outline-none transition resize-none" placeholder="..."></textarea>
                  </div>
                  <button type="submit" disabled={status === 'sending'} className="w-full py-4 bg-blue-900 dark:bg-blue-700 text-white rounded-xl font-bold text-lg hover:bg-blue-800 dark:hover:bg-blue-600 transition shadow-lg flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed">
